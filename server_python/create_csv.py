...
def write_custom_csv(emotions=['01', '02', '03', '04', '05', '06', '07'], train_name="train_custom.csv", test_name="test_custom.csv",
                    verbose=1):
    """
    ...
    for category in emotions:
        # train data
        for i, file in enumerate(glob.glob(f"data/train-custom/{category}-*.wav")):
            print(file)
        ...
        # test data
        for i, file in enumerate(glob.glob(f"data/test-custom/{category}-*.wav")):
